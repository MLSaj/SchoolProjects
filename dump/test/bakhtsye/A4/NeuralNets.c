...
int train_1layer_net(double sample[INPUTS],int label,double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
  ...
  int max_val = 0;
  for(int i = 1; i < 10; i++){
    if(activations[i] > activations[max_val]){
      max_val = i;
    }
  }
  //printf("Max val is %d\n", max_val);
  ...
  //backprop_1layer(double sample[INPUTS], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_io[INPUTS][OUTPUTS])
  //void feedforward_1layer(double sample[785], double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS], double activations[OUTPUTS])
  		// <--- This should return the class for this sample
...
int classify_1layer(double sample[INPUTS],int label,double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
  ...
  return max_val;   	// <---	This should return the class for this sample
}
  ...
  */ 
   ...
   ******************************************************************************************************/
   for(int i = 0; i < 10; i++){
     ...
     for(int j = 0; j < 785; j++){
       activation_val += sample[j]*weights_io[j][i]*SIGMOID_SCALE;
     }
     activations[i] = sigmoid(activation_val);
   }
...
void backprop_1layer(double sample[INPUTS], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_io[INPUTS][OUTPUTS]){
   ...
   */
    ...
    * ************************************************************************************************/
    ...
    double target_activations[10] = {0};
    for(int f = 0; f < 10; f++){
      if(sigmoid == logistic){
        target_activations[f] = 0;
      }
      else{
        target_activations[f] = -1;
      }
    }

    target_activations[label] = 1;
    ...
    //weight update is  w[i][j] = w[i][j] + learningrate*(derivative of error/with respest to w[i][j])
    //w[i][j] += (Output(a)*derivative_of_sigmoid*(tgt-output))*learning_rate
    //
    for(int i = 0; i < 10; i++){
     double activation_val = 0;
     for(int j = 0; j < 785; j++){
       ...
       if(sigmoid == logistic){
         ...
         activation_derivative = 1 - pow(activations[i],2);
       }
       weights_io[j][i] += ALPHA * (sample[j]) * SIGMOID_SCALE * activation_derivative * (target_activations[i] - activations[i]);
     }
   }
...
int train_2layer_net(double sample[INPUTS],int label,double (*sigmoid)(double input), int units, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS])
{
  ...
  * 	     across the wholne network
  *          
  *          You will need to complete feedforward_2layer(), backprop_2layer(), and logistic() in order to
  *          be able to complete this function.
  ***********************************************************************************************************/
  double activations[OUTPUTS] = {0};
  double hidden_activations[MAX_HIDDEN] = {0};

  feedforward_2layer(sample,sigmoid, weights_ih, weights_ho, hidden_activations, activations,units);

  backprop_2layer(sample,hidden_activations, activations,sigmoid,label,weights_ih, weights_ho,units);
  ...
  }		// <--- Should return the class for this sample  
...
void feedforward_2layer(double sample[INPUTS], double (*sigmoid)(double input), double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS], double h_activations[MAX_HIDDEN],double activations[OUTPUTS], int units)
{
  ...
  */ 
  ...
  // first layer of feedforward
   for(int i = 0; i < units; i++){
     double h_activation_val = 0;
     for(int j = 0; j < INPUTS; j++){
       h_activation_val += weights_ih[j][i] * sample[j]* SIGMOID_SCALE;
     }
     h_activations[i] = sigmoid(h_activation_val);
   }

  //second layer of activatin
  for(int i = 0; i < OUTPUTS; i++){
    double activation_val = 0;
    for(int j = 0; j < units; j++){
      activation_val += weights_ho[j][i] * h_activations[j] *SIGMOID_SCALE *(MAX_HIDDEN/units);
    }
    activations[i] = sigmoid(activation_val);
  }
...
void backprop_2layer(double sample[INPUTS],double h_activations[MAX_HIDDEN], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS], int units)
{
   ...
   */
    ...
    * ************************************************************************************************/
    ...
    //make a target for the last layer weight updates

    double target[OUTPUTS] = {0};
    ...
    //make a copy of the original weights of last layer
    double weights_ho_copy[units][OUTPUTS] = {0};

    for(int i = 0; i < units; i++){
      for(int j = 0; j < OUTPUTS; j++){
        weights_ho_copy[i][j] = weights_ho[i][j];
      }
    }

    //updating weights for the last layer
    for(int i = 0; i < OUTPUTS; i++){
      for(int j = 0; j < units; j++){
        double derivative;
      ...
      if(sigmoid == logistic){
        ...
        derivative = 1 - pow(activations[i], 2);
      }
      //w_ab = w_ab + (outA *dervative_of_activation *(target-output))*alpha
      weights_ho[j][i] += ALPHA *(h_activations[j]) * derivative * (target[i] - activations[i]) * SIGMOID_SCALE * (MAX_HIDDEN/units);
    }
    }
    ...
    for(int i = 0; i < units; i++){
      for(int j = 0; j < INPUTS; j++){
        double h_derivative;
        if(sigmoid == logistic){
          h_derivative = h_activations[i] * (1 - h_activations[i]);
        }
        else{
          h_derivative = 1 - pow(h_activations[i],2);
        }
        //error_a with respect to outputa = wab *EcB + wac*Erc(error from all neurons)
        double error_neurons = 0;
        for(int k = 0; k < OUTPUTS; k++){
          double activation_derivative = 0;
          if(sigmoid == logistic){
            activation_derivative = activations[k] * (1 - activations[k]);
          }
          else{
            activation_derivative = 1 - pow(activations[k],2);
          }
          error_neurons += ((target[k] - activations[k]) * activation_derivative * weights_ho_copy[i][k]);
        }

        weights_ih[j][i] += ALPHA * sample[j] * SIGMOID_SCALE* (MAX_HIDDEN/units) * h_derivative * error_neurons;
      }
    }
...
double logistic(double input)
{
 ...
 return sig;	// <--- Should return the value of the logistic function on the input 
}
